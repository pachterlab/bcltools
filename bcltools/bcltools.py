...
def get_n_reads(fastq):
    """Count the number of lines in a FASTQ file and divide by 4. Returns the number of reads"""
...
def make_bcl_files(path, read_len, n_reads):
    """
    Create initialized bcl files. There are read_len number of bcl files. Each has a header corresponding to the number of reads.
    """
    bcls = []
    header_fmt = "<i"
    header = pack(header_fmt, n_reads)
    ...
    with gzip.open(fastq, 'rb') as f:
        ...
        for lidx, l in enumerate(f, 0):
            ...
            if seq_bool and qual_bool:
                seq_bool = False; qual_bool = False
...
def bcl2fastq(file_path):
    ...
    with open(file_path, "br") as f:
        up = unpack(header_fmt, f.read(4))
        print(up)
        itr = iter_unpack(seq_fmt, f.read())
        for idx, i in enumerate(itr):
            base = num2base.get(3 & i[0], 0)
            qual = i[0]>>2
            print(bin(i[0]),"\t", base, "\t", qual)
